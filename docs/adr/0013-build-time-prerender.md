...
- **Documents grow, and compression absorbs it.** `napkin-battle`'s index goes from 24.4 KB to 36.9 KB raw, 6.0 KB to 8.6 KB gzipped. The markup is mostly repeated StyleX class names, which is close to the best case for a compressor. This is only affordable because compression is on at the edge — see the compression section of `//infra/cloud/gcp/lb:README.md`.
