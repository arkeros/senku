...
- **Stale objects are deleted, and a publish applies in three waves.** Content-addressed chunks would otherwise accumulate forever, one hash per deploy. The ordering is not a preference. Content-addressed objects are written first, because `index.html` names them and a client that fetched a new `index.html` before its chunks had landed would resolve a script tag to nothing; the mutable objects follow; the deletes come last, because a client that parsed the *old* `index.html` holds URLs for the old chunks and a delete that overtook an upload would resolve one of those to nothing. One barrier between the upload waves is enough because of reachability rather than reference structure — the chunks do name each other — since a hash written in the first wave is either new, and so named by nothing yet published, or already present and byte-identical. The barriers also buy failure atomicity: a publish that dies partway leaves the site coherently old rather than serving a page that names objects nobody uploaded. [ADR 0010](./0010-content-addressed-webroot.md) narrows the second wave to `index.html` alone.
