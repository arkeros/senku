...
- **Concurrent publishes to one bucket still interleave, and nothing serialises them.** Hashing shrinks the blast radius — two overlapping publishes now race on one object rather than twelve — but a lock would need a leased GCS object with stale-lock recovery, and the failure mode of getting that wrong is a wedged deploy path. Deliberately not built. Each app has its own bucket and its publish is a deploy node ordered after its own root, so overlap requires two concurrent deploy runs.

  What *is* prevented is a stale plan undoing a newer one. A publish decides what to retire and delete from a listing, then acts on that decision later, and in between another run may have rewritten the object. Both writes carry a `GenerationMatch` precondition on the generation the listing saw, so a rewritten object is left alone rather than dated or deleted. Without it an older run could remove a route object a newer run had just uploaded, leaving a declared route 404ing until somebody published again — a persistent wrong state, where interleaving otherwise only produces a transient one. A skipped object is reported, and the next publish replans from what it finds; no replanning happens mid-run, because a run that lost a race has nothing useful to say about the object that beat it.
