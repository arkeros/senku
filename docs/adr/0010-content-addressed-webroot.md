...
- **The publish's wave boundary is still required, and gets sharper.** `index.html` names a hashed entry bundle, so the content-addressed wave must still complete before the entry document is written. What changes is the second wave's size — from twelve objects to essentially one — which is what turns "the site is coherently old or coherently new" from an argument into a property.
