"""`tf_runner` — the rule emitted by `tf_root` for each `<name>.<verb>`.

Replaces an earlier `sh_binary(args = [...])` macro. The args injection
...
load("//terraform/toolchain:toolchain.bzl", "TerraformInfo")

_TOOLCHAIN_TYPE = "//terraform/toolchain:toolchain_type"
...
def _tf_runner_impl(ctx):
    ...
    if first_gen.short_path.startswith("../"):
        fail("tf_runner: `generated` files must live in the consumer workspace, got " + first_gen.short_path)
...
tf_runner = rule(
    ...
    attrs = {
        ...
        "_template": attr.label(
            default = "//terraform:runner.sh.tpl",
            allow_single_file = True,
        ),
        "_run_sh": attr.label(
            default = "//terraform:run.sh",
            allow_single_file = True,
        ),
