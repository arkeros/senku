...
Used by consumers (e.g. bifrost terraform modules) for `lint` (sh_test) and
`validate` (sh_binary) targets. Replaces a prior `args = ["$(rootpath
...
_ATTRS = {
    ...
    "_template": attr.label(
        default = "//terraform:lint_wrapper.sh.tpl",
        allow_single_file = True,
    ),
}
