...
the `@<install_name>` hub repo's generated BUILD. Each instance carries
everything `tf_root` needs to build a hermetic per-root working
directory:
...
tf_provider_target = rule(
    ...
    attrs = {
        ...
        "archives": attr.string_keyed_label_dict(
            mandatory = True,
            allow_files = True,
            doc = "platform -> filegroup label (one of the `_provider_archive_repo` outputs from the install extension).",
        ),
    },
