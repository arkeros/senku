 ...
 * `.d.ts`); the client bundle will switch to `.client.js` in roadmap
 * step 7.
 *
 * Wires:
 *   - `createStaticHandler(routes)`     — runs the matched-route chain.
 *   - `renderToPipeableStream(...)`     — streams the React tree out.
 *   - Hono `serveStatic` for `/assets/*` and `/*.css` (matches the
 *     stub server's prefixes from step 5; not yet route-aware).
 *
 * Out of scope at step 6 (later commits):
 *   - `route.lazy` switch + modulepreload (step 7)
...
let manifestFile, outServer, appTitle;
