...
// renderToReadableStream emits everything from <!DOCTYPE> on. Hydration
// data + client bundle script live in <body> rather than <head> to
// avoid React 19's auto-hoisting reshuffling them; step 9 reintroduces
// head-level <title>/<meta> via the matched-chain meta flow.
function Document({
  children,
  hydrationData,
  ...
  try {
    ...
    const stream = await renderToReadableStream(
      <Document hydrationData={hydrationData}>
        {/*
          The <Suspense> wrapper here mirrors the one in client_main.tsx
          so React's hydration model lines up: the streamed HTML carries
          the boundary's resolved content + its hydration markers, and
          the client's lazy chunk resolves into the same boundary
          without triggering a mismatch (error #418).
        */}
