...
# Synthesize .terraformrc with the absolute mirror path of THIS host.
# filesystem_mirror.path requires an absolute path; the bazel-bin abs
# path is per-host and not knowable at Bazel build time, so the rc
# file is a runtime artifact rather than a bazel output. Lives in
# $WORK so manual `terraform init` works after a `bazel run`
# materializes it; bazel doesn't track the file (not a declared
# output), so subsequent rebuilds don't touch it.
if [[ -d "$WORK/_providers" ]]; then
  # `rm -f` first because the previous run's file may be mode 555 — the
  # bazel action environment that wrote it on first invocation strips
  # write bits. `cat > existing-readonly` would fail with EACCES;
  # `rm -f` only needs write on the parent dir (which stays writable).
  rm -f "$WORK/.terraformrc"
  cat > "$WORK/.terraformrc" <<EOF
# Generated by tf/run.sh. Pinning .terraformrc here lets terraform
# init resolve providers from ./_providers without any network
# fallback.
provider_installation {
  filesystem_mirror {
    path    = "${WORK}/_providers"
