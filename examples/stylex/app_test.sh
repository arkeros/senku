...
grep -q '"LOCALE": "en"' "$ENV_DEV" || { echo "FAIL: dev env.js missing LOCALE literal"; exit 1; }
...
grep -qF '"LOCALE": atob("${LOCALE_B64}")' "$ENV_TPL" \
  || { echo "FAIL: env.js.tpl missing atob(\${LOCALE_B64}) placeholder"; exit 1; }
